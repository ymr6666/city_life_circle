...
# 2. 修改 docker-compose.yml 中的数据库密码、暴露端口（可选）
...
```
┌─────────────────────────────────────────────┐
│  service: db   postgis+pgRouting 13 镜像     │  ← 挂载 ./data 卷，启动时自动 pg_restore dump
│  service: app  python:3.11 + waitress        │  ← 挂载 ./data，启动时自动 pg_restore dump
│         │                                     │
│         └── 同一镜像内：API（:5000/api）+ 静态页面（frontend/dist）  │
└─────────────────────────────────────────────┘
```

> 数据卷持久化：`docker/volumes/db`（PostgreSQL 数据）与 `docker/volumes/cache`（人口瓦片）。
> 升级/迁移时直接备份这两个卷，或重新执行 `pg_dump`。
