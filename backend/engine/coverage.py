...
from .walk_layer import _polygon_area_m2
...
def blindzone_grid(category, mode="walk", time_budget_min=15,
                   ...
                   point_mode=False, max_points=15000):
    ...
    # 2. bbox 内该类别 POI (供前端展示/点击)
    facilities = execute_query("""
        SELECT p.id, p.name, p.address, p.sub_category,
               ST_X(p.geometry), ST_Y(p.geometry),
               COALESCE(p.rating, ''), COALESCE(p.opentime_today, '')
        FROM hefei_poi p
        WHERE p.category = %s
    ...
    """, (category, *bounds))
    facility_pts = [{
        "id": r[0], "name": r[1], "address": r[2], "sub_category": r[3],
        "lng": float(r[4]), "lat": float(r[5]),
        "rating": r[6], "opentime_today": r[7],
    } for r in facilities]
